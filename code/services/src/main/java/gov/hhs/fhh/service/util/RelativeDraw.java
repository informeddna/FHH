...
public class RelativeDraw extends Relative {
    ...
    public byte[] organizeFamilyTree(final Person self, Map<Disease, String> abbreviationMap) throws Exception {
        try {
            ...
            Set<Relative> probandsChildren = getChildren(proband);
            // organize my own kids and grandkids) as descendant of dad
            for (final Relative myKid : probandsChildren) {
            ...
            if (numBrothers == 0 && numSisters > 0) {
                diagramShift = (probandsChildren.size() - 2) / 2 * (PERSONSIZE + SIBLINGSPACING) + 10;
            }
            ...
            final int width = paternalGrandfather.getChildrenWidth(paternalGrandfather)
                    ...
                    + PERSONSIZE + SIBLINGSPACING + SPOUSEDISTANCE + halfSiblingSpace + widthSpacer + diagramShift;
